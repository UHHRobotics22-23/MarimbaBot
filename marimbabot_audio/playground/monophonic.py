...
# TODO, adapt it to marimba
def states_to_marimba(states: list, note_min: str, hop_time: float) -> list:
    """
    Converts state sequence to an intermediate, internal marimba notation
...
#TODO adapt it to marimba
# def marimba_to_midi_with_midiutil(bpm: float, marimba: list) -> midiutil.MIDIFile():
#     """
...
#     quarter_note = 60 / bpm
#
#     onsets = np.array([p[0] for p in marimba])
#     offsets = np.array([p[1] for p in marimba])
#
...
#         midi.addNote(
#             0, 0, int(marimba[i][2]), onsets[i], durations[i], 100)
#
#     return midi
def marimba_to_midi_with_pretty_midi(bpm: float, marimba: list) -> pretty_midi.PrettyMIDI():
    """
    Converts an internal piano roll notation to a MIDI file
    ...
    for i, _ in enumerate(onsets):
        inst.notes.append(pretty_midi.Note(velocity, int(marimba[i][2]), onsets[i], onsets[i]+durations[i]))
...
def wave_to_midi(
        ...
        frame_length: int = 2048,
        hop_length: int = 512,
        note_min: str = "C4",  # lowest note for marimba
        note_max: str = "C7",  # highest note for marimba
        p_stay_note: float = 0.9,
        p_stay_silence: float = 0.7,
        pitch_acc: float = 0.9,
        voiced_acc: float = 0.9,
        onset_acc: float = 0.9,
        spread: float = 0.2) -> pretty_midi.PrettyMIDI():
    ...
    Args:
        ...
        note_min (str, optional): Lowest allowed note in "A#4" format. Defaults to "C4".
        note_max (str, optional): Highest allowed note in "A#4" format. Defaults to "C7".
    ...
    marimba = states_to_marimba(states, note_min, hop_length / srate)
    bpm = librosa.beat.tempo(y=audio_signal)[0]
    midi = marimba_to_midi_with_pretty_midi(bpm, marimba)
...
if __name__ == '__main__':
    import os
    print("Starting...")
    data_root = "/home/wang/workspace/sound2midi/test_data"

    file_in = os.path.join(data_root, "110_Fm_MarimbaAlpMel_331_01.wav")
    file_out = "./out.mid"
    y, sr = librosa.load(path=file_in, sr=None)
    print("Audio file loaded!")
    midi = wave_to_midi(audio_signal=y, srate=sr)
