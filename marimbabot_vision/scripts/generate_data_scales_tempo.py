...
class LilypondGenerator():
    ...
    """
    def add_major_minor_scales(self, voice, note, mode):
    ...
    def generate_piece(self, scale, mode):
        ...
        if self.scale:
            self.add_major_minor_scales(voice_1, scale, mode)
...
def generate_sample(i, args, scale, mode):
    lilypondGenerator = args.lilypondGenerator
    string, staff = lilypondGenerator.generate_piece(scale, mode)
...
if __name__ == "__main__":
    ...
    for mode in ["major", "minor"]:
        for music_note in ['c', 'd', 'e', 'f', 'g', 'a', 'b']:
            generate_sample(i, args=args, scale=music_note, mode=mode)
            i += 1
